...
def render_character(character, font_path=None, index=0, folder="character_images", font_size=random.randint(8, 64), image_size=(64, 64), text_color=(0, 0, 0), bg_color=(255, 255, 255)):
    """
    ...
    position = (random.randint(0, max(image_size[0] - text_width - 10, 0)), random.randint(0, max(image_size[1] - text_height - 30, 0)))
    ...
    save_path = os.path.join(folder, f"char_{index}_{font_name}_{character}.png")
...
for i in range(70000):
    ...
    render_character(character, font_path=font_path, index=i, folder=folder)  # Render the character with the selected parameters
